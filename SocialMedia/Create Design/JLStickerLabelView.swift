...
//        adjustsWidthToFillItsContens(self)
    ...
    private func setup() {
        ...
        // Close Static Info
        NotificationCenter.default.addObserver(self, selector: #selector(self.methodOfCloseReceivedNotification(notification:)), name: Notification.Name("NotificationCloseIdentifier"), object: nil)
    ...
    func setupTextLabel(selectedLabel: String) {
        self.globalInset = 19
        ...
        self.setupLabelTextView()
        customSetupLabelTextView(selectedLabel: selectedLabel)
        self.setupBorder()
    ...
    func setupImageLabel(selectedImage : UIImage) {
        self.globalInset = 19
        ...
        self.setupImageView(selectedImage : selectedImage)
        self.setupBorder()
...
extension JLStickerLabelView: UIGestureRecognizerDelegate, adjustFontSizeToFillRectProtocol {
    ...
    //MARK: - - - - - Method for receiving Data through OverPlay Post Notificaiton - - - - -
       @objc func methodOfReceivedNotificationOerPlay(notification: Notification) {
           print("Value of notification : ", notification.object ?? "")
           let yourColor = (notification.object! as! UIColor)
           imageView?.image = imageView?.image?.tintImage(with: yourColor)
       }
...
extension JLStickerLabelView {
    ...
    private func setupLabelTextView(selectedLabel:String) {
        labelTextView = JLAttributedTextView(frame: self.bounds.insetBy(dx: globalInset!, dy: globalInset!))
        labelTextView?.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        labelTextView?.clipsToBounds = true
        labelTextView?.delegate = self
        labelTextView?.text = selectedLabel
        labelTextView?.backgroundColor = UIColor.clear
        labelTextView?.tintColor = UIColor(red: 33, green: 45, blue: 59, alpha: 1)
        labelTextView?.isScrollEnabled = false
        labelTextView?.isSelectable = true
    ...
    private func customSetupLabelTextView(selectedLabel:String) {
           labelTextView = JLAttributedTextView(frame: self.bounds.insetBy(dx: globalInset!, dy: globalInset!))
           labelTextView?.autoresizingMask = [.flexibleWidth, .flexibleHeight]
           labelTextView?.clipsToBounds = true
           labelTextView?.delegate = self
...
//         labelTextView?.isEditable = false
    ...
    private func setupImageView(selectedImage : UIImage) {
...
//        imageView?.backgroundColor = UIColor.red
        enableZoom()
        print("selectedImageselectedImageselectedImageselectedImageselectedImage",selectedImage)
    ...
    private func testNotification(){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.methodOfReceivedNotificationOerPlay(notification:)), name: Notification.Name("OverPlayNotificationIdentifier"), object: nil)

        
        
     }
     //MARK: - - - - - Method for Close Static Info receiving Data through Post Notificaiton - - - - -
     @objc func methodOfCloseReceivedNotification(notification: Notification) {
         print("Value of notification : ", notification.object ?? "")
         self.removeFromSuperview()
